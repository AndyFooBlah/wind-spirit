...
CATS = ['firstspring', 'routine', 'crisis', 'visitor', 'expansion', 'spirit', 'dream']
LABEL = {
...
NCASES = max(len(json.load(open(f))['results']) for f in glob.glob(os.path.join(ROOT, 'docs', 'evals', 'results', '*.json')))
BASELINE = next((r['score'] for r in runs if r['model'] == 'gemini-3.8-flash'), 0.97)
...
NUDGE1 = {'Gemini 3.5 Flash-Lite (OR)': (-8, -11, 'right'), 'MiniMax M2.7': (5, -22),
          'Claude Sonnet 5': (5, -12), 'GPT-5.6 Luna': (-6, 12, 'right'), 'Gemini 3.8 Flash (Vertex)': (-6, 4, 'right'),
          'Llama 4 Maverick': (5, -12), 'GLM 5.3 Flash': (5, -12), 'Nemotron 3 Ultra': (5, -12), 'Gemini 3.1 Pro (Vertex)': (-6, 4, 'right'),
          'Gemini 2.5 Flash': (5, 6), 'GPT-5.4 nano': (5, -12), 'Gemma 4 26B (Vertex)': (5, 6), 'Gemini 3.1 Flash-Lite': (-6, -12, 'right'), 'GPT-5 nano': (-6, 4, 'right'), 'Gemini 2.5 Flash-Lite (OR)': (5, -16), 'Qwen3 235B (Vertex)': (5, 6), 'Gemini 3.8 Flash (OR)': (5, 6), 'GPT-5 mini': (5, -12), 'Kimi K3': (-6, 4, 'right')}
...
ax.set_xscale('log'); ax.set_xlabel('cost per decision, tenths of a cent (log scale)'); ax.set_ylabel(f'rule score, mean over {NCASES} cases'); ax.set_ylim(0.88, 1.004)
ax.axhline(BASELINE, color='#999', linestyle=':', linewidth=1); ax.text(0.33, BASELINE - 0.004, 'Gemini 3.8 Flash baseline', fontsize=8, color='#666')
...
ax.axvline(10, color='#b3261e', linestyle='-', linewidth=1.2); ax.text(10.4, len(lat) - 1.5, '10 s: out, whatever the score', fontsize=8, color='#b3261e', va='center', fontweight='bold')
slow = sum(1 for r in lat if r['ms'] > 10_000)
...
ax.set_xlabel('seconds per decision (log scale)'); ax.set_title(f'Latency: the axis that disqualified {slow} models'); ax.grid(axis='x', color='#eee'); ax.tick_params(axis='y', labelsize=8)
