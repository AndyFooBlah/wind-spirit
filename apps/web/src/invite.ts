...
async function call(path: string, init: RequestInit): Promise<Response> {
  const t = await idToken(); if (!t) throw new Error('Could not sign in. Check your connection and reload.');
  return fetch(`${PROXY_URL}${path}`, { ...init, headers: { 'content-type': 'application/json', authorization: `Bearer ${t}`, ...(init.headers ?? {}) } });
}
