...
export function buildView(w: World, v: Village, o: ViewOpts): VillageView {
  ...
  const parties = w.parties.filter(p => p.home === v.id).map(p => ({ kind: p.kind, size: p.members.length, weeksOut: 0, destination: p.targetVillage !== undefined ? (w.villages[p.targetVillage]?.name ?? '?') : directionOf(w, v.tile, p.target), status: p.waiting > 0 ? 'waiting for an answer' : p.returning ? 'coming home' : p.kind === 'expedition' && p.at === p.target ? `collecting ${cname(p.gather?.c ?? '')}` : 'travelling' }));
